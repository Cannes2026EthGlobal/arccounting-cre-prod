...
var fetchEthPrice = (sendRequester, config) => {
...
var writePriceOnChain = (runtime2, priceUsd) => {
  const { consumerAddress, chainSelectorName, gasLimit } = runtime2.config;
  ...
  runtime2.log(`ETH/USD price: $${priceUsd} → stored as ${priceBigInt} (8 decimals)`);
  ...
  runtime2.log(`Price written on-chain! txHash: ${bytesToHex(result.txHash ?? new Uint8Array(32))}`);
};
var onCronTrigger = (runtime2, payload) => {
  runtime2.log("Cron triggered — fetching ETH price...");
  const httpCapability = new cre.capabilities.HTTPClient;
  const priceData = httpCapability.sendRequest(runtime2, fetchEthPrice, ConsensusAggregationByFields({
    priceUsd: median
  }))(runtime2.config).result();
  runtime2.log(`Consensus ETH price: $${priceData.priceUsd}`);
  writePriceOnChain(runtime2, priceData.priceUsd);
